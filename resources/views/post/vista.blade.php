    ...
    <section class="w-full md:w-2/3 flex flex-col px-3">
        ...
        <article class="flex flex-col shadow my-4">
            ...
            <div class="bg-white flex flex-col justify-start p-6">
                ...
                <h1 class="text-3xl font-bold hover:text-gray-700 pb-4">{{ $post->titulo }}</h1>
                <p href="#" class="text-sm pb-8">
                    By <a href="#" class="font-semibold hover:text-gray-800">{{ $post->user->name }}</a>,
                    Published on {{ $post->fechaFormateada() }} | {{$post->human_read_time}}
                </p>
                ...
                @if ($prev)
                    <a href="{{ route('view', $prev) }}" class="block w-full bg-white shadow hover:shadow-md text-left p-6">
                        <p class="text-lg text-blue-800 font-bold flex items-center"><i
                            ...
                            Previous</p>
                        <p class="pt-2">{{ \Illuminate\Support\Str::words($prev->titulo, 5) }}</p>
                    </a>
                @endif
            </div>
            <div class="w-1/2">
